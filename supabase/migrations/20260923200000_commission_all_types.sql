-- Foreman commission on auction-first peer awards (was skipped).
...
begin
  ...
  -- Legacy name: peer bidÃ·n path (auction only). Collection/cash gates use award_first.
  ...
  if p_method = 'fixed' and c.type = 'loan' and c.current_cycle >= c.duration then
    raise exception 'No new loans on the last month â€” collect dues and settle leftover cash instead';
  end if;
  ...
  -- Auction-first: book the winnerÃ¢â‚¬â„¢s bidÃƒÂ·N share as paid-in (self-contribution).
