-- Sacrifice-hand fixed subtype: early winners take pot − half instalment;
...
begin
  ...
  if hand_sacrifice then
    -- Half an instalment left as cash dividends (full pot for last member).
    if last_hand then
      sacrifice := 0;
    else
      sacrifice := floor(public._base_instalment(c) / 2);
    end if;
    discount := sacrifice;
