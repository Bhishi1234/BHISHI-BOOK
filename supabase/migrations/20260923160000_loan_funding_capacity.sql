-- Cap loan face at cash on hand + still-expected collections this month.

create or replace function public._loan_funding_capacity(p_chit uuid)
returns numeric
language plpgsql
stable
as $$
...
begin
  ...
  select coalesce(sum(amount), 0) into money_in from public.payments where chit_id = p_chit;
  ...
  cash := greatest(0, money_in - money_out);

  for m in
    select customer_id, slot from public.chit_members where chit_id = p_chit
  loop
    due := public._raw_due_hand(p_chit, m.customer_id, m.slot, c.current_cycle);
    paid := public._paid_in_cycle(p_chit, m.customer_id, c.current_cycle, m.slot);
    remaining := remaining + greatest(0, due - paid);
  end loop;
...
grant execute on function public._loan_funding_capacity(uuid) to authenticated;


-- settle_payout: clamp loan face to funding capacity
