-- Fix award-first fixed/lucky/hand-sacrifice: do not withhold current (or prior) hapta
-- from the pot payout. Hapta is collected after the award. Loans unchanged.
...
begin
  ...
  -- Award-first (non-loan): pot from organiser float; collect hapta after award.
  -- Do not withhold unpaid dues (would double-count: lower payout AND still-due hapta).
  -- Loans still net outstanding against the disbursement.
  if p_method = 'settlement' or auction_peer or (award_first and c.type <> 'loan') then
    arrears := 0;
  else
